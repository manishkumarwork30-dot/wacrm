...
console.log("Environment variables loaded. ENCRYPTION_KEY exists:", !!process.env.ENCRYPTION_KEY);

async function run() {
  // Use dynamic imports to prevent hoisting
  const { NextRequest } = await import('next/server');
  const { POST } = await import('../src/app/api/whatsapp/webhook/route');

  console.log("Simulating webhook call to POST (dynamic import)...");
