...
filepath = r"c:\Users\DELL\Downloads\sendgb-Fws4BeDcEzn\code\whataappforl\src\lib\whatsapp\chatbot.ts"

with open(filepath, "r", encoding="utf-8") as f:
    content = f.read()

# 1. Define simplified templates
old_templates_block = """  // Load message templates with fallback to default Hindi values
...
new_templates_block = """  // Load message templates with fallback to default Hindi values
...
if old_templates_block in content:
    content = content.replace(old_templates_block, new_templates_block)
    print("Replaced templates block.")
else:
    # Try with raw string carriage returns
    old_templates_block_rn = old_templates_block.replace("\\n", "\n").replace("\n", "\r\n")
    new_templates_block_rn = new_templates_block.replace("\\n", "\n").replace("\n", "\r\n")
    # Clean normalizations
    content_norm = content.replace("\r\n", "\n")
    old_norm = old_templates_block.replace("\\n", "\n")
    if old_norm in content_norm:
        content_norm = content_norm.replace(old_norm, new_templates_block.replace("\\n", "\n"))
        content = content_norm.replace("\n", "\r\n")
        print("Replaced templates block using normalization.")
    else:
        print("Error: Message templates block not found in file.")

# 2. Modify Trigger state (welcomeMsg with buttons)
old_trigger = """      // Send greeting
      await sendAndLogBotMessage(conversationId, phoneNumberId, accessToken, senderPhone, welcomeMsg);
      return true; // consumed"""

new_trigger = """      // Send greeting with buttons
...
if old_trigger in content:
    ...
    print("Replaced trigger block.")
else:
    content_norm = content.replace("\r\n", "\n")
    old_trig_norm = old_trigger.replace("\r\n", "\n")
    if old_trig_norm in content_norm:
        content_norm = content_norm.replace(old_trig_norm, new_trigger.replace("\r\n", "\n"))
        content = content_norm.replace("\n", "\r\n")
        print("Replaced trigger block using normalization.")
    else:
        print("Error: Trigger block not found.")

# 3. Replace state switch case logic
# Let's target from case 'AWAITING_LAND_CONFIRMATION': all the way to the end of processChatbot.
# To do this safely, let's write a targeted replace for the states.

old_switch_body = """  switch (currentState) {
...
new_switch_body = """  switch (currentState) {
    case 'AWAITING_LAND_CONFIRMATION': {
...
# Clean normalization for checking
content_norm = content.replace("\r\n", "\n")
old_switch_norm = old_switch_body.replace("\\n", "\n").replace("\r\n", "\n")
new_switch_norm = new_switch_body.replace("\\n", "\n").replace("\r\n", "\n")

if old_switch_norm in content_norm:
    content_norm = content_norm.replace(old_switch_norm, new_switch_norm)
    content = content_norm.replace("\n", "\r\n")
    print("Replaced switch body successfully.")
else:
    print("Error: Switch body block not found in file.")
